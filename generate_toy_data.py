...
PARSER.add_argument(
    ...
    help="Type of dataet to generate. One of \"copy\" or \"reverse\" or \"sort\"")
PARSER.add_argument(
    "--output_dir",
    type=str,
...
def make_sorting(num_examples, min_len, max_len):
  """
  Generates a dataset where the target is equal to the source sorted.
  Sequence lengths are chosen randomly from [min_len, max_len].
  ...
  for _ in range(num_examples):
    ...
    source_tokens = np.random.choice(
        list(VOCABULARY), size=turn_length, replace=True)
    target_tokens = sorted(source_tokens)
    yield " ".join(source_tokens), " ".join(target_tokens)
